...
Note that `tidytable` functions do not use `data.table`'s modify-by-reference functionality.
...
mutate_marks <- bench::mark(
  ...
  data.table = data.table:::shallow(test_dt)[, ':='(double_a = a * 2, a_plus_b = a + b)],
  check = FALSE,
  iterations = iters)
...
case_marks <- bench::mark(
  ...
  tidytable = dt_mutate(case_dt,
                     test = dt_case(x < median_x, "low",
                                    ...
                                    is.na(x), "unknown",
                                    default = "other")),
  data.table = data.table:::shallow(case_dt)[,':='(test = fifelse(x < median_x, "low",
