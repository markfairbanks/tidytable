...
Group by calls are done from inside any function that has group by functionality (such as `summarize.()` & `mutate.()`) by using the `.by` argument.
...
Below is some example `tidytable` code that utilizes `.by` that we'll then compare to its `dplyr` equivalent. The goal is to grab the first two rows of each group using `slice.()`, then add a row number column using `mutate.()`:
...
In this case both `slice()` and `mutate()` will operate "by group". This happens until you call `ungroup()` at the end of the chain. Note that the `ungroup()` call is unnecessary in `tidytable`.
