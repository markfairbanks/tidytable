...
`tidytable` started as a complement to the [dtplyr](https://github.com/tidyverse/dtplyr) package, since `dtplyr` is missing some of `dplyr`'s functionality and doesn't cover any `tidyr` functions. However `dplyr`'s core verbs (mutate, arrange, etc.) are now included in `tidytable` to allow the user access to all functions in one place.
