...
In this case both `mutate()` and `slice()` will operate "by group". This happens until you call `ungroup()` at the end of the pipe chain.

However `data.table` doesn't "remember" groups between function calls, so this code would be written like this in `tidytable`:
