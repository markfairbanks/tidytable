...
summarize_across..data.frame <- function(.df, .cols = everything(), .fns, ...,
  ...
  if (!is.list(.fns)) {
    ...
    if (!is.null(.names)) summarize_across_warning()
...
summarize_across_warning <- function() {
  warn(glue("Warning: As of tidytable v0.5.4, column names generated by summarize_across.() follow \\
            dplyr summarize(across()) conventions of using suffixes instead of prefixes. This may \\
            require updates to older versions of your code."),
       .frequency = "regularly",
       .frequency_id = "summarize_across_names")
}
