...
left_join_marks <- bench_mark(
  tidyverse = left_join(left_tbl, right_tbl, by = c("c", "d")),
  dtplyr = as.data.table(left_join(lazy_dt(left_dt), lazy_dt(right_dt), by = c("c", "d"))),
...
inner_join_marks <- bench_mark(
  tidyverse = inner_join(left_tbl, right_tbl, by = c("c", "d")),
  dtplyr = as.data.table(inner_join(lazy_dt(left_dt), lazy_dt(right_dt), by = c("c", "d"))),
...
unnest_marks <- bench_mark(
  tidyverse = unnest(unnest_tbl, list_column),
...
nest_marks <- bench_mark(
  tidyverse = group_nest(nest_tbl, c, d),
  dtplyr = as.data.table(nest(lazy_dt(nest_dt), data = c(c, d))),
