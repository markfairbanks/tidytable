...
summarize_marks <- bench::mark(
  tidyverse = summarize(group_by(test_tbl, c, d), avg_a = mean(a), .groups = "drop"),
  dtplyr = as.data.table(summarize(group_by(lazy_dt(test_dt), c, d), avg_a = mean(a))),
  tidytable = summarize.(test_dt, avg_a = mean(a), .by = c(c, d)),
  data.table = test_dt[, list(avg_a = mean(a)), by = .(c, d)],
  check = FALSE, iterations = iters, memory = FALSE, filter_gc = FALSE, time_unit = 'ms') %>%
  mutate(expression = as.character(expression),
...
wider_marks <- bench::mark(
  ...
  dtplyr = collect(pivot_wider(lazy_dt(wider_dt), names_from = name, values_from = value)),
