...
* `case.()` & `dplyr::case_when()` both can have a default value that is a vector, whereas `data.table::fcase()` can not. Testing used a default value of length 1 due to this constraint.
* Where timings are missing for `dtplyr`, this is due to certain verbs not yet being implemented in the package; these includes `pivot_wider()`, `unnest()`, among others.
...
all_marks <- all_marks %>%
  ...
  select(function_tested, data.table, dtplyr, tidytable, tidyverse, pandas, tidytable_vs_tidyverse)
```
